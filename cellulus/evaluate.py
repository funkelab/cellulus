...
def evaluate(inference_config: InferenceConfig) -> None:
    ...
    f_postprocessed = zarr.open(inference_config.segmentation_dataset_config.container_path)
    ds_postprocessed = f_postprocessed[inference_config.segmentation_dataset_config.dataset_name]
    ...
    print(f"The mean F1 score is {np.mean(F1_list)} and mean SEG score is {np.mean(SEG_list)}")
...
def compute_pairwise_IoU(prediction, groundtruth):
    ...
    IoU_table = np.zeros((len(prediction_ids), len(groundtruth_ids)),
            dtype=np.float32)
    for j in range(len(prediction_ids)):
        for k in range(len(groundtruth_ids)):
            intersection = (prediction==prediction_ids[j]) & (groundtruth==groundtruth_ids[k])
            union = (prediction==prediction_ids[j]) | (groundtruth==groundtruth_ids[k])
