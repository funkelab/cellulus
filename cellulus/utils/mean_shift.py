...
def mean_shift_segmentation(embedding_mean, embedding_std, bandwidth,
        min_size):
    embedding_mean = torch.from_numpy(embedding_mean)
    if embedding_mean.ndim == 4:
        embedding_mean[:, 1] += torch.arange(embedding_mean.shape[2])[None, :,
                None] # += dy
        embedding_mean[:, 0] += torch.arange(embedding_mean.shape[3])[None,
                None, :] # += dx
    elif embedding_mean.ndim == 5:
        embedding_mean[:, 2] += torch.arange(embedding_mean.shape[2])[None, :,
                None, None]
        embedding_mean[:, 1] += torch.arange(embedding_mean.shape[3])[None,
                None, :, None]
        embedding_mean[:, 0] += torch.arange(embedding_mean.shape[4])[None,
                None, None, :]
    mask = (threshold_otsu(embedding_std) > embedding_std)
    mask = mask[None]
    segmentation = segment_with_meanshift(embedding_mean,
                                 bandwidth,
                                 mask=mask,
...
def segment_with_meanshift(embedding,
                           ...
                           cluster_all):
    anchor_mean_shift = AnchorMeanshift(bandwidth,
...
class AnchorMeanshift():
    ...
    def compute_masked_ms(self, embedding, mask=None):
        if embedding.ndim == 3:
            ...
            if mask is not None:
                ...
                if mask.sum() == 0:
                    return -1 *np.ones(mask.shape, dtype=np.int32)
                reshaped_embedding = embedding.permute(1, 2, 3,
                        0)[mask].view(-1, c)
            else:
    ...
    def __call__(self, embedding, mask=None):
        ...
        for j in range(len(embedding)):
            ...
            mean_shift_segmentation = self.compute_masked_ms(embedding[j], mask=mask_slice)
